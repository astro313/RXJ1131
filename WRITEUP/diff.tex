...
\new{redshifted to $\nu_{\rm obs}$\eq139.256\,GHz towards RXJ1131 at the CARMA \cco redshift of 
$z_{\rm CO(3-2)}$\eq0.655}
...
at $\gtrsim$\,27$\sigma$ \old{significance, confirming} \new{significance. Based on this measurement, we refine} the redshift \old{at} \new{of RXJ1131 to} \old{$z_{\rm CO}$\eq0.6537\,$\pm$\,0.0005.}
\new{$z_{\rm CO}$\eq0.6541\,$\pm$\,0.0002\mbox{\footnote{This redshift is derived by fitting a double-Gaussian to
...
\new{The \cco spectrum appears to be consistent with a double-peaked profile, as shown in \Fig{co32spec}, where we over-plot spectra of the \bco and \cco lines.}
\new{We extract the peak fluxes and their corresponding uncertainties for the blue and red wing independently.
We find a peak line flux of 5.13\pmm1.43 Jy\,\kms beam\pmOne for the blue wing, 
indicating a $\gtrsim$3$\sigma$ detection for this component, and a peak line 
flux of 11.45\pmm1.99\,Jy\,\kms beam\pmOne for the red wing, indicating a $\sim$\,6$\sigma$ detection. 
}
We estimate a line intensity of
\old{35.7\,$\pm$\,21.9 Jy\,\kms} \new{35.7\,$\pm$\,6.9\,Jy\,\kms (\Tab{obsprop})} by summing up fluxes over the FWZI
...
\new{0.78\,$\pm$\,0.37, where this uncertainty includes
...
The velocity scale is \old{with respect to \mbox{$z$=0.6537, which corresponds to the
dynamical center of the \bco line.}} \new{same as in \Fig{CO21spec}).} The spectral resolution for \cco and \bco
...
emission, which we here assume to be dominated by the foreground galaxy. The peak 
\old{flux (0.39\,$\pm$\,0.08\,mJy)}
\new{emission}
in the residual map coincides with the lensed arc, \old{and} \new{with 
a flux density of $S_{\nu}$\eq0.39\,$\pm$\,0.08\,mJy.}
\new{This peak flux} is consistent with
the difference between the integrated and the peak flux in the
original continuum map ($\sim$0.4 mJy).
We therefore adopt \old{$S_{\nu}$ = 0.39\pmm0.08 mJy} \new{$S_{\nu}$\eq0.39\pmm0.12 mJy} as the 2\,mm continuum \old{emission towards} \new{flux measurement for}
the background galaxy (RXJ1131).
The quoted uncertainty is \old{from the flux measurement in the residual map, }
\new{derived by adding those associated with the flux density of the
point-source model ($\delta S_{\nu}$\eq0.088\,mJy) and the peak flux in the residual map (0.08 mJy) in 
quadrature,}
and does not account for that in the
...
5\,GHz and 2\,mm.
\new{The spectral slope derived for the background source is flatter than the typical slope of pure synchrotron emission \mbox{\citep[$\alpha$\ssim$-$0.7; ][]{Andreani93a}}, suggesting 
...
emission (red) overlaid on} \new{around the labelled central velocity where
...
magnification factor varies from 10.9 to 7.8 \mbox{\citepalias{Claeskens06a}}, indicating that
...
\new{As discussed in \mbox{\Sec{caveat}}, we interpret RXJ1131 as a disk galaxy as it displays
...
\caption{Left: 
\old{Source-plane positions from best-fit \mbox{\bco} lens models are indicated with their associated uncertainties atop
...
Here, we use a flux density of $S_{\rm 2mm}$\eq0.27\pmm0.08mJy derived in \Sec{deblend}
...
dust mass of 16\petm{5}{12}\E{8}\,\Msun, \old{uncorrected for lensing} \new{all of which are not lensing-corrected}.
...
dust mass $M_{\rm dust}$ of \old{\mbox{11\petm{5}{6}\E{8}\,\Msun}} \new{\mbox{14\petm{5}{7}\E{8}\,\Msun}}, all of which are \old{uncorrected for lensing}\new{not lensing-corrected}.
...
Thus, large velocity offsets between BLR and NLR lines comparable to that of RXJ1131 are uncommon but 
...
\new{The observed velocity offset between the BLR and NLR lines may be explained
by a recoiling black hole (BH),}
...
SDSS 0956+5127 at \mbox{$z$\,$\sim$\,0.714 \citep{Steinhardt12a}}\old{. One notable system is}\new{, and} SDSSJ0927+2943 at \mbox{$z$\,$\sim$\,0.713}\old{, where}\new{In the case of 
SDSSJ0927+2943, the black hole recoil scenario was proposed based on the 
velocity offset between}
a set of redshifted narrow lines and a second set of blueshifted broad AGN lines
\old{have been observed} \mbox{\citep{Komossa08a}}\old{, similar to the case for RXJ1131}.
However, this \old{scenario} was recently refuted by \mbox{\citet{Decarli14a}}, who find that the
...
e.g., outflow/inflow of gas in the BLR, rotation of the accretion disk, viewing 
angle towards the accretion disk, and 
...
\mbox{\citet{Sluse07a}} report a covering factor of 20\% for the accretion disk in RXJ1131
...
\new{With the existing data, we do not favor a particular scenario mentioned above as the interpretation for 
...
but the optical BLR lines are redshifted by \old{$\sim$780\,\kms}\new{$\sim$715\,\kms}.\old{This implies that the AGN is offset from the dynamical center of its host galaxy.
...
The latter scenario might also explain the high black hole spin parameter reported by 
\mbox{\citet[$a$\ssim0.9][]{Reis14a}}.
