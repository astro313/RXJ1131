...
\input{preamble}
\slugcomment{To be submitted to the ApJ, with changes from the referee report }
 \makeatletter
...
\title{Molecular Gas Kinematics and Spectral Energy Distribution Properties of the Strongly-Lensed Quasar Host Galaxy RXS J1131$-$1231}
...
beam size of 3\farcs2 $\times$ 1\farcs9 at a position angle (PA) of 8\degr for the lower sideband
...
% In the image plane, the *convolved* size is 6.6"+/-0.5" x 4.5"+/-0.3".
...
3\farcs8\pmm0\farcs4$\times$1\farcs9\pmm0\farcs4 and 3\farcs6\pmm0\farcs3$\times$1\farcs5\pmm0\farcs3, separated by $\sim$2\farcs2 in RA and $\sim$1\farcs7 in Dec.  % RA: 3-0.76; Dec: 2.49-.78
...
5\,GHz and 2\,mm.
The spectral slope derived for the background source is flatter than the typical slope of pure synchrotron emission \citep[$\alpha$\ssim$-$0.7; e.g.,][]{Andreani93a}. This likely suggests % {\bf DR: 3C273 flat-spectrum radio source BLAH}
...
The best-fit lens models obtained here for different CO channel slices show that differential lensing also plays
...
dust mass of 16\petm{5}{12}\E{8}\,\Msun, none of which are corrected for lensing magnification.
...
In addition, a SFE and a sSFR similar to those of high-$z$ disk 
...
% \oiii is forbidden, must be from low-density region (further out)
...
This together with a symmetric source-plane velocity gradient argues for a rotating disk in RXJ1131, in good agreement with previous findings \citepalias{Claeskens06a}. Physical quantities derived for RXJ1131 and the companion throughout this paper are summarized in \Tab{prop}.
...
This picture is in agreement with that based on the \mbox{$M_{\rm BH}$$-$$M_{\rm bulge}$} relation,
...
Theoretical studies have suggested that the negative feedback from an AGN may remove a 
large fraction of molecular gas from its host galaxy, thereby quenching 
its star formation \mbox{\citep[e.g., ][]{DiMatteo05a}}.
...
% spin up scenario:
% Wang+16: """ The spin of a BH is widely believed to play a crucial role in
% determining the radio emission from SMBHs. The scenario that
% an energy is extracted from an SMBH being spun up by a BH?
% BH merger (e.g., Chiaberge & Marconi 2011) is favored for
% high-mass SMBHs. This spin-up scenario is, however, almost
% infeasible for low-mass SMBHs because they are believed to
% largely build from secular evolution driven by internal
% dynamical processes in the disk growth rather than from two
% BHs merging (e.g., Kormendy & Ho 2013).
