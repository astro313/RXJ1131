...
Our lens modeling shows that the asymmetry of the double-horned \bco line profile
...
the quasar host galaxy is an extended rotating disk, with a CO size of $R$$\sim$6\,kpc in radius and
...
We decompose the lensing-corrected total \bco line flux into two components and 
find a lensing-corrected gas mass of $M_{\rm gas}$\ssim1.4\E{10}\,\Msun for RXJ1131
and \ssim2\E{9}\,\Msun for the companion, corresponding
to a gas mass ratio of $\sim$7:1.
The gas mass fraction of $\sim$19\% inferred for RXJ1131 is consistent with the trend of decreasing molecular gas fraction since $z$\ssim1 reported in recent studies.
The lensing-corrected stellar mass $M_{*}$\ssim3\E{10}\,\Msun,
dust mass $M_{\rm dust}$\ssim4\E{8}\,\Msun, IR luminosity $L_{\rm IR}$\ssim1.5\E{12}\,\Lsun, and 
star formation rate SFR$_{\rm FIR}$\eq120\pmm63 \sfrU of RXJ1131
together with its source-plane morphology of the gas and stellar populations
indicate ongoing star-formation activity that maybe as extended as the molecular gas reservoir.
We find a $M_{\rm BH}$$/$$M_{\rm bulge}$ ratio of 0.27\petm{11}{8}\% that is higher
than the local value
but consistent with other galaxies at this epoch. Collectively, our results suggest
that the bulk of the black hole mass of RXJ1131 is already in place 
while its stellar bulge component is still assembling, and that its major mode of \SF is likely merger-driven.
\end{abstract}
...
only unresolved CO observations have been carried out for $\sim$$30$ ultra-luminous infrared galaxies (ULIRGs) up to date \citep{Combes11a, Combes13a}. %18+15
...
% the $M_{\rm BH}$$/$$M_{\rm bulge}$ ratios of high-$z$ submillimeter galaxies (SMGs) 
% are lower than those of local AGN host galaxies \citep[\eg][]{Borys05a,Alexander08a}, 
% whereas those of high-$z$ quasar host galaxies 
% are higher than the local values \citep[\eg][]{Walter04a, McLure06a,Peng06a, Riechers08a,Coppin08a}, 
% suggesting that the co-eval growth of high-$z$ AGNs and their host galaxies follows different 
% timescales compared to their local counterparts. % different AGN mechanism, different AGN accretion rate
% However, any links between such fast-growing SMBHs and their host galaxies are not yet established at these high redshifts.
high-$z$ AGN host galaxies do not follow the same $M_{\rm BH}$$-$\mbulge relation \citep[\eg][]{Walter04a,Borys05a, McLure06a,Peng06a, Riechers08a,Coppin08a,Alexander08a}.
Yet, the $M_{\rm BH}$$-$\mbulge relation remains poorly-constrained 
at intermediate redshifts.
This is owed to the difficulties in measuring stellar emission in optical quasar host galaxies, where
...
different source-plane locations and of different extent is being magnified and stretched out by different amount.
The lensed emission is therefore spatially resolved over more beams which reduces the sidelobes and enables 
measurements of the stellar emission in the host galaxy.
...
In this paper, we present \bco and \cco line data observed towards RXJ1131 and
broadband photometry spanning rest-frame UV to radio wavelengths to 
study the properties of the molecular gas, the dust and stellar populations in the 
host galaxy of RXJ1131.
...
applied to an image taken with the \hst ({\it HST}) by aligning it to
a radio image taken with the \vla (VLA).
...
$21.5$ \kms).  {\bf DR: DSB, 320 MHz/128 channels? <-> 2.5 MHz? I did not bin the data}
...
nearby ULIRGs and high-$z$ quasars \citep[e.g.,][]{Weiss07a, CW13}, and also 
...
We detect PdBI 2\,mm continuum emission, as shown in \Fig{cont}. The integrated flux density is
...
emission towards the foreground galaxy. The peak flux (0.39\,$\pm$\,0.08\,mJy)
...
The quoted uncertainty is from the flux measurement in the residual map and do not account for that in the de-blending procedure, where we have assigned 100\% of the point source flux to the foreground galaxy.
We report the peak flux in the original map 
($S_{\nu}$\,=\,799\pmm88\,$\mu$Jy\,beam\pmOne) for the foreground galaxy.
We note that this peak flux density is only our best estimate at the resolution of our observations, a non-negligible contribution from the background source to the total integrated (blended) flux may be present.
...
%
The first use of a dynamical lens modeling approach is presented by \citet{Riechers08a}, who reconstruct a source-plane
...
The $V$-band dynamical mass is substantially lower than the gas mass, which
...
We thus classify the system as a {\em ``wet'' minor merger}.
...
The wider CO linewidth observed in RXJ1131 also supports the merger picture.
...
\subsubsection{Gas Mass Fractions}
...
Recent studies find that the baryonic gas fraction of starburst galaxies has decreased from $\sim$40\% to $\lesssim$10\% between $z$\ssim2 and $z$\ssim0 \citep[\citetalias{Solomon97a};][]{GS04a, Tacconi06a},
% On the other hand,
...
normal ``star-forming'' galaxies \citep{Geach11a, Saintonge11a, Tacconi13a}\footnote{These authors use  the ``Galactic'' \alphaco conversion factor to compute the molecular gas mass.}.
...
We note that this gas mass is physically feasible based on the dynamical mass constraints derived in \Sec{dyn}.
...
The specific star formation rate (sSFR\eq SFR/$M^*$) of 4\petm{2.6}{2.4}\,Gyr\pmOne derived for RXJ1131
is $\lesssim$1.5$\sigma$ above the main sequence according to
the redshift-dependent ``main sequence" relation in \citet[and references therein]{Tacconi13a} but is
consistent with those of the \cite{dacunha10a} sample of local ULIRGs, suggesting that the 
majority of \SF in RXJ1131 could be driven by its interactions with the companion galaxy, 
unlike most disk galaxies despite sharing similar \SF rate, \SF efficiency, and CO disk size.
...
grow faster and/or earlier than their host galaxies at higher redshifts \citep[e.g., ][]{Walter04a, Peng06a, McLure06a}.
...
suggest that the system is a ``wet'' minor merger.

Fitting dust SED models to the IR-to-mm photometry, we derive
lensing-corrected dust mass of $M_{\rm dust}$\ssim4\E{8}\,\Msun,
infrared luminosity of \LIR$\sim$\,1.5\E{12}\,$(5.5/\mu_{\rm L})$\,\Lsun, 
...
whereas regions of recent \SF maybe embedded within the molecular gas reservoir as a result of 
...
maybe heavily dust-obscured. 
Hence, the true extent of recent \SF maybe as extended as the molecular gas 
reservoir.
...
However, we find a sSFR\ssim4 Gyr\pmOne that is $\lesssim$1.5$\sigma$ higher than the main sequence, suggesting 
...
$M_{\rm BH}$$/$$M_{\rm bulge}$\eq0.27\petm{11}{8}\% for  of RXJ1131, providing
...
consistent with the results from existing studies of the evolution in the growth of SMBHs and their host galaxies
 ...
 %the ratio is larger at high z than the present day, implying evolution in the Mbh?Msph relation
