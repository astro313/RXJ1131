...
\defcitealias{Solomon97a}{S97}\defcitealias{Greve05a}{G05}
...
While we cannot draw conclusions any evolution in the
molecular gas size
...
and its intrinsic source size of $\sim$700\,pc obtained from the {\em HST} $V$-band lens model \citepalias{Brewer08a},
we find a dynamical mass of $M_{\rm dyn}$\,$\sin^2 i$ = 5\E{6}\,\Msun.
This corresponds to $M_{\rm dyn}$\,=\,2\E{7}\,\Msun if we assume an inclination angle of 30$\degr$.
...
\citep[\eg][\citetalias{Riechers11a,Ivison11a}]{SV05a}.
% gas-to-dyn ratio
...
This implies that the gas fraction in ULIRGs at $z$\,$<$\,1 is
similar to those in high-$z$ starburst galaxies
...
as well as high-$z$ ``normal'' star-forming disc galaxies
\citep[assuming an \alphaco conversion factor of 0.8 \alphaU;][]{Daddi10a,Tacconi10a}.
...
40\pmm34, which is significantly lower than the average
in the \citetalias{Combes13a} sample of $f_{\rm gas-dust}$\,=\,206, but
consistent within the scatter and also with the
sample of local ULIRGs studied by \citet[][]{Wilson08a},
with a ratio of $($50$-$70$)$.
...
We note that the gas-to-dust ratio is only good within
within a factor of 2$-$3 as
differential lensing may exist between the gas and the dust, leading to
an under- or overestimation of the dust mass via our adoption of the CO magnification factor.




% We also note that the gas-to-dust ratio of ULIRGs
% varies widely in the literature, ranging from 50 to $>$200.
% Such a variation can arise depending on a few factors, such as the resolution

% Alternately, this may indicate a different \alphaco conversion factor.

% The primary source of uncertainty in the gas mass is
% undeniably the conversion factor, which if all galaxy samples we
% are comparing against use the same conversion factor but that our dust-to-gas
% ratio is different (due to the Mdust), which implies different physical
% conditions.
% It thus appears that RXJ1131 is more dusty than ULIRGs at the same epoch, which may indicate different
% ISM conditions/ISM properties and potentially a different alphaCO.
% Then it might be reasonable that the alphaco = 0.8 is inappropriate in this case. A higher alpha CO (upper limit using disc value) lead to a gas mass that is $\sim$5 factors higher.
% Which seems likely to account for the discrepancy because Galactic conversion factor \citep{Papadopoulos12a} might indeed be the true conversion factor in RXJ1131.

% The secondary effect/uncertainty is the excitation condition, which we have conservatively assumed to be thermalized.
% Should a subthermal condition, we would obtain an even
% higher gas mass) and that ULIRGs have thermalized excitation up to $J$ = 3 or
% 4, so the assumption on the brightness temperature ratio should not be the
% main contribution to the discrepancy in GDR. Also how dust mass
% is highly dependent on the method (model and how the data are obtained, e.g.
% resolution) used to obtain the dust mass (e.g. Seaquist+04, finding a GDR
% affecting by a factor of $\sim$5 in the inner few kpc in some cases for
% U/LIRGs but global GDR unaffected: ). For now, assume that the dust mass
% obtained from the literature is the best (most appropriate, close to physical)
% way to obtain dust mass with the given data for their sample. Then we discuss
% the implications/reasoning/causes for a low SFE in RXJ1131.
...
In principle, the gas consumption timescale or the star formation efficiency (SFE) furnishes information on the AGN and star-formation
triggering mechanisms.
Physically, the SFE depends on the SMBH gas
consumption time scales and on the energy injected
to the ISM by AGN feedback. High SFE may thus be indication
of a small gas reservoir that was reduced by BH accretion and outflows.
In practice, the SFE is the ratio between the \fir luminosity and CO luminosity as an indicator of the SFR per solar mass of molecular gas.


Using the far-IR luminosity definition used by
C13 (40\,$-$\,500\,$\micron$),
we find a SFE $($=\LFIR/$M_{\rm gas})$ of 58\pmm10\,\Lsun$/$\Msun,
or equivalently a depletion
timescale of $\tau$\,=\,102\pmm25\,Myr.
% Gas depletion time assuming the SF is the only way to consume the gas reservoir, and that SF continues at the current rate. Since it is likely that
% SFR will vary in such a merger situation and that the AGN will consume some of the gas, hence the gas depletion timescale should only be considered as an upper limit.
% Such an SFE is lower than other U/LIRGs (SFE: 180 +/- 160 Lsun/M \citep{Solomon97a, Combes11a}.
% The depletion timescales in the C13 sample
% of 39 galaxies between 0.2$<$$z$1.0 are $<$ 100 Myr.
% C13: SFE = 100 Lsun/Msun
...
% At high redshifts,
% SMGs the irregular, turbulent star-bursting galaxies (SFR $\sim$1\E{2}-1\E{3}\,\Msun yr\pmOne) and BzKs are the ``normal" star-forming disk galaxies (at z$\sim$1.6) with relatively lower SFE, SFR ($\sim$BLAH) and
% longer depletion time (BLAH Myr) (CW13, \citealt{Daddi10a}), but surface
% densities, SFRs, velocity dispersion very similar to local mergers (Daddi10a, Krumholz\&Dekel10, Genzel+11).
% SFE is highly dependent on the alphaCO if using ratio between the \LIR and gas mass instead of using the ratio between (far-)IR luminosity and CO luminosity.
% And the use of which alphaCO is also not well justified, especially at high-z (\ie high uncertainties on the alphaco value).

% % galactic value: 4.6
% In fact
% e.g. Daddi10a argue for an \alphaco conversion factor of 3.6\pmm0.8 for normal SF disk (BzK galaxies) at z$\sim$1.5 using kinematic analysis/dynamical model (based on the dynamical mass). Whereas \citet{Tacconi08a, Carilli10a} place an upper limit of $\sim$0.8 on high-z SMGs, leading the emergence of two star formation modes.
% Carilli+10: based on dynamical constraints


% if RXJ is intriscally low SFE:
% In terms of more recent simulations, it may be what Hopkins09a suggests that the disc can survive merge by lowering SFE in low-mass systems, allowing them to retain large gas fraction and distributing the gas to larger radii.
% Thus in Hopkins09a: 1:1 gas-riich mergers can yeild disc-dominated remantns, and 1:3-1:4 ratio major mergers can yield B/T < 0.2.

% This unexpectedly low SFE in RXJ1131...
% % that it is indeed converting gas into stars at a lower rate.
% but as mentioned above, the possible scenario that an \alphaco of 0.8 UNIT is inappropriate here.


% \subsubsection{Conversion Factor $\alpha_{\rm CO}$}

% From the gas-to-dust ratio and SFE, it is evident that a different \alphaco conversion factor may be more appropriate for RXJ1131.


% The physical motivation/basis for deriving the gas mass from CO luminosity is
% assuming virialized (self-gravitating), non-overlapping gas cloud.
% Then \alphaco scales as $n^{1/2}/T_b$, where $n$ is the average H$_2$ number density
% in the clouds and $T_b$ is the intrinsic brightness temperature of the CO line.


% Downes,Solomon \& Radford 93, Solomon97: CO emitting gas in the centers of ULIRGs is unlikely to be virialized, but instead bound by the potential of the galaxy or from molecular gas in pressure rather than gravitational equilibrium.

% DS98: showed that alphaco is a factor of $\sim$6 smaller for local ULIRGs than for local spirals.
% Also see Scoville+97


% % also a function of metallicity (Leroy11 apj 737, 12), excitation, pressure.
% %\alphaco lower for objects with extended surface gas densities (Downes Solomon98), where gas temperature and velocity dispersion are higher.
% %While the conversion factor $(\alphaco)$ is commonly adopted as 0.8  (K \kms pc$^2$)\pmOne
% %for ULIRGs, its numerical value depends on the ISM environment (Narayanan11a).
% %For instance, it could be
% %different in a dynamically turbulent galaxy, where the gas is warmer, more excited, and denser.
% % Instead of obtaining a crude estimate
% %using the dust mass and CO luminosity, assuming a
% %gas-to-dust ratio. {\textbf but this gal. have very different GDR than others, can I even assume that? or is it
% %because of the alphaCO that is leading to the different in GDR, but others also uses the same alphaCO.
% %Also probably shouldn't do this given how poorly-constrained the Mdust is..}
...
% %We find $\alpha_{\rm CO}$ = BLAH\pmm$(f_{\rm gas-dust}/72)$ (K \kms pc$^2$)\pmOne, where the uncertainty does not include systematic uncertainties
% %in the dust absorption coefficient $(\kappa)$ and the gas-to-dust ratio.
% % We discuss a potential different \alphaco that is more appropriate
% % for RXJ1131.
...
% %smaller (Downes et al. 1993; Bryant \& Scoville 1999). This can
% %arise in the ULIRGs if the gas is concentrated in the nuclear
...
% %nuclei as a result of the intense star formation activity, and AlphaCO
% %should vary as density and temperature e.g. (Dickman et al. 1986).
% % Also should be a continuous distribution instead of a bimodal value (Narayana+12).
...
% In fact, physically, In a minor merger, the rise in gas temperatures and velocity dispersions are not as extreme as they are in major mergers. Hence, the rise in velocity-integrated CO intensity is not as large, and the X-factor (and thus alphaco) does not decrease as much {Narayana11a}.

% In contrast to what was found in the classical study by DS98, who find that
% adopting a MW XCO for their sample of nearby ULIRGs would cause the gas mass
% to exceed the Mdyn (and thus inappropriate for their sample), the Mgas of RXJ1131 merger system derived using MW XCO is
% consistent with the dynamical mass.
% DS98: Gas masses were derived from radiative transfer model instead of using standard conversion factor, which yields the \alphaco = 0.8 UNIT. f$_{gas-dyn}$ = 1/6.

% % how would our conclusions change if we use different alphaCO
% If we instead apply a galactic \alphaco conversion factor of 4.6, the gas mass would increase by a factor of $\sim$6, and longer depletion time.
...
(broad Mg{\sc II}$\lambda$ 2798$\Angstrom$ and narrow lines [O{\sc III}]$\lambda\lambda$ 4959, 5007$\Angstrom$ emission, Sluse03a)
...
There are a few reasons to this e.g. SB/AGN driven winds or AGN outflows, jets, gravitational recoil of BH in post-merger. % (Comerford & Greene, 2014, ApJ, 789, 112).
...
% Other objects seen with such big velocity offset:
