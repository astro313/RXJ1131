...
Common interpretations invoked to explain offsets between BLR and systemic redshifts include
...
Alternatively, the observed offset %between the BLR and the systemic redshift (and the NLR)
...
% the AGN (with a BLR dynamical timescale $\sim$10\E{2} years \citep{Bonning07a}, \ie can be quickly regenerated), 
% whereas this condition is not necessary for NLRs.
%
% In hierarchical model of galaxy formation, supermassive black holes are expected to form through
...
$v_{\rm kick}$\ssim4000\,\kms for spinning BHs, % \citep[\eg][]{Campanelli07a}, 
...
% ----------------------------------------
%A recoiling black hole may therefore 
%oscillate around the center of the galaxy as its orbit is decaying due to dynamical friction, 
%or leave its host galaxy if $v_{\rm kick}$$\gg$$v_{\rm esc}$ and dynamical friction is inefficient 
%\citep[see \eg \citealt{Volonteri10a} for a discussion]{Guedes09a,Bonning07a, Colpi14a}. 
%% escape velocity
%If we assume a \citet{Hernquist90a} profile for the stellar bulge in RXJ1131, 
%%$M_{\rm BH}$/\mstar\ssim1/375, $v_{\rm circ}$\ssim$v_{\rm rot}, 
%% then the upper limit of $v_{\rm kick}$\ssim2$\times$$v_{\rm circ}$, 
%% bulge escape velocity $v_{\rm esc}$\eq2.83$\v_{\rm circ}$ = 858\kms
%% %(in the case of a density profile of the spherical \citet{Hernquist90a} model for the stellar bulge, 
%% %spherically symmetric isotropic model)%  \citep{BoylanKolchin04a}, 
%then the escape velocity $v_{\rm esc}$\eq2$\sqrt{2}$$v_{\rm circ}$, where $v_{\rm circ}$ is the velocity
%at $r$\eq$a$, the stellar bulge scale length that is at most a few kpc.
%Assuming that the rotation curve of RXJ1131 is similar to that MW, then at inner radius ($\lesssim$ few kpc), 
%we would expect $v_{\rm rot} (r=a)\gtrsim$$v_{\rm rot}$ of the extended gas disk (see \Sec{dynamics}).
%Given that we don't know precisely the stellar bulge scale length, if we assume 
%$v_{\rm rot} (r=a)$\ssim$v_{\rm rot}$ of the gas disk, then $v_{\rm esc}$\ssim860\,\kms. 
%Therefore, the recoil BH in RXJ1131 is unlikely to escape its gas-rich host galaxy.%  \citep[\eg][]{Loeb07a, Volonteri10a}.
