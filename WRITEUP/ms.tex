...
according to the overall median in the preliminary models, as listed in \Tab{lens}. The corresponding masses within the Einstein radii is
...
The best-fit magnification factors are listed in \Tab{model}.
We compare our model parameters in \Tab{lens} describing
the mass distribution of the lensing galaxy
against that of the SIE model presented by C06. The position
%\footnote{We note that they report a positional offset
% relative to the light centroid in the HST image, which are at slightly
%different astrometry. Our comparison takes into
%account the astrometric correction applied.}
of lensing mass, Einstein Radius, PA and axial ratio
are all consistent within the uncertainties.
% PA = -73.6, ellipticity = 0.45,
% their theta\_R = 1.84" $\pm$ 0.01.
...
can be found in the observed (image plane) velocity field and
the double-horned line profile in \Fig{CO21spec}. Furthermore,
the reconstructed source plane emission in optical-NIR
is also best-reproduced using a $n$\,=\,1 Sersic profile (C06).
Our interpretation therefore tends in favor of a disk morphology for RXJ1131.

% the companion
One interesting result from our lens model is that a better fit is
...
separated by $\sim$2.4\,kpc away from the AGN host galaxy.

We emphasis that the spatial resolution of the data at hand
...
factors is likely under-predicted. We will discuss this further in the following section. % magnification factor is underpredicted, see Bussmann15
...
represented by magenta ellipses (see text in \Sec{lensmodel} for caveats).
...
Lens modeling of this source from previous studies find non-negligible
differential lensing effect across H-Band, V-band, and I-band. The
magnification factor ranges from 10.9 to 7.8, where the decrease is caused
