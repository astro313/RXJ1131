...
\begin{abstract}
We report observations of the \bco and \cco line emission towards
the quadruply-imaged quasar
RXS\,J1131$-$1231 at $z$\,=\,0.654 obtained using the
\pdbi (PdBI) and \carma (CARMA), making this the first spatially resolved
CO study at intermediate redshift.
Our lens modeling shows that the highly asymmetric double-horned line profile
observed in the \bco spectrum is mainly a result of differential lensing, where
the magnification factor varies from $\sim$3 to $\sim$8 across
different kinematic components.
The intrinsically symmetric line profile with a clear velocity
gradient in the source-plane suggests that
the quasar host galaxy is a rotating disk.
...
The lensing-corrected gas mass of
$M_{\rm gas}$\ssim1.4\E{10}\,\Msun inferred for RXJ1131
and \ssim2\E{9}\,\Msun for its companion correspond
to a gas mass ratio of $\sim$7:1.
The extended CO gas disk in RXJ1131 ($R$$\sim$6\,kpc) yields a dynamical mass
of $M_{\rm dyn}$\ssim8\E{10}\,\Msun, and thus a gas mass fraction of $\sim$19\%,
which is consistent with the trend of decreasing molecular gas fraction since $z$\ssim1.
The lensing-corrected $M^{*}$\ssim3\E{10}\,\Msun,
$M_{\rm dust}$\ssim4\E{8}\,\Msun, $L_{\rm IR}$\ssim1.5\E{12}\,\Lsun, and SFR$_{\rm FIR}$\eq120\pmm63 \sfrU of RXJ1131
together with the source-plane morphology of the gas and the stellar populations
suggest that ongoing \SBs are embedded within the molecular gas reservoir,
and that the nuclear \SBs are likely triggered by
interactions with the companion.
The $M_{\rm BH}$$/$$M_{\rm bulge}$ of $\gtrsim$0.27\% derived for RXJ1131 lies above the
local relation but is consistent with other radio-loud galaxies at this epoch, suggesting
that the co-evolution between RXJ1131 and its host galaxy is different than those of local AGN host galaxies.
\end{abstract}
...
Studies of galaxy evolution have been focusing on the interplay between
\SF and active galactic nucleus (AGN) activity across cosmic epochs.
It is not well-understood when and how stellar populations of present-day galaxies were assembled,
...
has been established locally, suggesting a co-eval growth between local SMBHs and their host galaxies.
...
the local relation \citep[\eg][]{Borys05a,Alexander08a} but that of high-$z$ quasar host galaxies is found
lying above
...
is also too small compared to that based on the local relation \citep{Walter04a, Riechers08a,Coppin08a}.
...
This is owed to the difficulties in measuring stellar emission in the host galaxy under the presence of a bright quasar, or alternatively
due to the lack of spatially resolved CO observations to provide dynamical constraints on the stellar mass.
...
allowing spatially resolved measurements their emission in a
relatively uncontaminated fashion.
...
We have thus carried out a detailed study of RXJ1131 as presented in this paper.
...
In \Sec{HST}, we describe the astrometric corrections
applied to align an {\it HST} image with an VLA radio image.
In \Sec{results}, we report measurements of the \bco and \cco lines and broadband
photometry spanning the optical to radio wavelengths.
In \Sec{anal}, we present dynamical lens modeling of the \bco data and
SED modeling of the host galaxy of RXJ1131 to
derive physical parameters reflecting the nature of this system.
In \Sec{diss}, we discuss the merger stage of RXJ1131,
the velocity offset between the AGN lines and the CO lines,
and the $M_{\rm BH}$$-$\mdyn relation of RXJ1131.
...
($\nu_{\rm rest}$\,=\,$230.538$ GHz) redshifted to $\nu_{\rm obs}$\ssim$139.0$\,GHz
...
a calibration accuracy of $\sim$35\% based on the flux scale uncertainties,
...
\caption{Continuum-subtracted spectrum of \bco emission toward RXJ1131. The velocity scale
...
We construct the zeroth order moment map, a red/blue channel map, and
...
We detect \cco line emission toward RXJ1131 in the CARMA data at $\gtrsim5\sigma$ significance.
...
Assuming the spatial extents of \bco and \cco is similar and therefore the emission is
...
We detect PdBI 2\,mm continuum, as shown in \Fig{cont}. The integrated flux density is
1.2\pmm0.2 mJy, with a peak flux
$S_\nu$\,=\,800\pmm88\,$\mu$Jy\,beam\pmOne
centered on the lensing galaxy. Slightly extended emission
along the lensed arc is also detected.
This suggests that we detect emission both in
the foreground galaxy and in the background galaxy and that the
...
To enusre adequate SNRs for lens modeling, we bin the frequency channels by a factor of five
...
above each respective model channel. Flux density on the y-axis is shown on a log scale.
\label{fig:delensed}}
\end{figure}
...
We thus classify the system to be a {\em ``wet'' minor merger}.
...
Top: each panel corresponds to a channel map of width $\sim$100\,\kms of the observed
...
In all panels, location of the foreground lensing galaxy is indicated by a black dot and
...
The best-fit model of the red-most channel (top left panel) contains two source components (see text for details).
\label{fig:model}}
\end{figure*}
...
Fitting two Gaussians with a single FWHM
...
The best-fit models are shown in \Fig{SED} along with the broadband photometry that are listed in \Tab{photometry}.
...
from the CO lens models. This yields a
 ...
 and
 a total IR luminosity of $\sim$1.5\E{12}\,\Lsun, implying RXJ1131 can be classified as an ULIRG.
...
to account for differential lensing.
We thus find a stellar mass of $M^*$\eq2.95\petm{1.32}{0.86}\E{10}\,\Msun, which
...
The resulting dust mass and \LIR are consistent with those obtained
...
% Assuming a \citet{Salpeter55a} IMF, we find a ``stellar mass build-up" time of $M^*$$/$SFR\ssim440 Myr for RXJ1131.
...
\caption{SEDs of RXJ1131 at $z$\eq0.654 and its lensing galaxy at $z$\eq0.295.
The photometry data (colored markers) are not corrected for lensing (see \Tab{photometry}).
Best-fit SED models of the thermal dust emission towards RXJ1131
are obtained assuming a MBB$+$power-law model.
The dashed (solid) line corresponds to a model with (without) MIR constraint at 24\,$\micron$, respectively.
\label{fig:SED}}
\end{figure}
...
Yet, given the merging nature of these galaxies,
...
high-$z$ starburst galaxies  \citep[e.g.,][hereafter G05]{Greve05a}
...
which is more
extended than the average of a sample of disk-like U/LIRGs studied by
...
This dynamical mass is substantially lower than the gas mass, and
% , which is a more reliable estimate based on the data in hand,
% we do not use this dynamical mass to derive other physical parameters.
% The underestimated dynamical mass may reflect
may be reflecting the dusty nature of the companion.
% in which the $V$-band source size to
% appear much smaller than its true extent.
...
local ULIRGs \citep{Wilson08a}, but lower in the Milky Way by
...
There are a number of systematic uncertainties associated with the derived gas-to-dust ratio. For instance,
...
we expect this to be a minor effect as CO emission in ULIRGs are typically thermalized up to $J$ = 3 or 4.
...
Assuming an \alphaco of 0.8\,\alphaU, this would imply that
...
will evolve into a disk galaxy with a small bulge component upon merging given its low SFE and the extended disk.
...
be required to transform the gas disk of RXJ1131 into a stellar spheroid and evolve into
an E/S0 galaxy.
Thus, RXJ1131 may retain its disk component and
...
with the optical quasar --- the point-like images along the lensing arc are
offset to the NW of the CO line center emission.
On the other hand, we find that the redshift of the [O\,{\scriptsize III}] lines tracing the narrow line region (NLR)
is fully consistent with the systemic redshift, supporting previous claims that [O\,{\scriptsize III}] lines are good proxies
...
% offset in this system: between a set of redshifted (narrow) versus a set of blueshifted (broad).
...
% The fact that the BH in RXJ1131 has a high spin parameter \citep[a\ssim0.9;][]{Reis14a}
% renders this scenario
% % the recoiling BH scenario
% a viable option for the origin of the blueshift.
...
Based on the black hole mass of \mbh$\sim$\,8\E{7}\,\Msun \citep{Sluse12a},
and adopting the stellar mass of $M^{*}$\,=\,2.95\petm{1.32}{0.86}\E{10}\,\Msun
as an upper limit to the bulge mass,
we find a $M_{\rm BH}$$/$$M_{\rm bulge}$ ratio of 0.19\%$-$0.38\%.
This is consistent with other intermediate-$z$ radio-loud AGNs \citep{McLure06a}
but lies above the local $M_{\rm BH}$$-$$M_{\rm bulge}$ relation \citep{HR04a}.
The elevated $M_{\rm BH}$$/$$M_{\rm bulge}$ ratio of RXJ1131 compared to local
AGNs is consistent with the emerging picture that quasars
grow faster than their host galaxies at higher redshifts.
% \ie $M_{\rm BH}$/$M_{\rm bulge}$\ssim0.15\% or equivalently $\log{M_{\rm BH}/M_{\rm bulge}}$\ssim$-$2.8
...
Taking into account the large phase errors associated with the \cco measurement, this
is consistent with thermalized excitation.
We also detect marginally resolved
2\,mm continuum underlying the \bco line in the PdBI data
and resolved radio continuum emission at 5\,GHz in the archival VLA data
in both the foreground lensing galaxy and RXJ1131.
Using
positional priors from an astrometric-corrected {\it HST} image,
we de-blend broadband photometry spanning the rest-frame UV to radio wavelengths
that we have compiled from public catalogues and previous optical studies, as well as from those we have
extracted using aperture photometry.

Based on our lens modeling analysis of different \bco velocity channels,
%  to study the intrinsic gas dynamics of RXJ1131.
...
% inferred from previous lens modeling in the optical wavelengths
...
The magnification factor inferred from our best-fit models is found to
vary from $\mu_{\rm L}$\ssim3 to $\sim8$ across channels. This is indicative of an extended molecular gas
distribution in the host galaxy of RXJ1131 and thus different kinematic components
of the gas are magnified inhomogeneously.
...
This together with a symmetric velocity gradient reconstructed in the source plane argues for a rotating disk morphology for RXJ1131, in good agreement with previous findings \citepalias{Claeskens06a}.
Assuming thermalized excitation between \aco and \bco and \alphaco=\,0.8\,\alphaU,
we find a lensing-corrected gas mass of $M_{\rm gas}$\,=\,$($1.38\pmm0.33$)$\,\E{10} \Msun
for RXJ1131
and $($1.92\pmm0.09$)$\,\E{9} \Msun for the companion.
This corresponds to a {\em gas} mass ratio of $\sim$7:1.
Using the source-plane \bco size of $R$\ssim6\,kpc for RXJ1131, we find a dynamical mass of $M_{\rm dyn}$\ssim8\E{10}\,\Msun,
% \,=\,(8.3\pmm1.9)\E{10}\,\Msun
% and $M_{\rm dyn}$\,=\,(3.5\pmm2.3)\E{9}\,\Msun for the companion,
and thus a gas mass fraction of $\sim$0.19.
This gas fraction is consistent with the trend of decreasing molecular gas content
since $z$\ssim1 \citep[e.g.,][]{Lagos11a, Combes13a} which has been suggested as the cause for
the decline in SFR towards $z$\ssim0 \citep[and references therein]{CW13}.
Due to the lack of stringent constraints on the individual galaxy mass of RXJ1131 and of the companion, we classify the system as a ``wet'' minor merger on the basis of their gas mass ratio.
%The merger system studied here is in stark contrast to the
%``wet-dry'' merger SMM J04135$+$10277 at $z$\eq2.8,
...
lensing-corrected $M_{\rm dust}$\ssim4\E{8}\,\Msun,
\LIR$\sim$\,1.5\E{12}\,\Lsun, % which indicates that the host galaxy can be classified as a ULIRG.
% We also derive a far-IR luminosity, which yields a SFR$_{\rm FIR}$\ssim120\,\sfrU.
and a far-IR luminosity that corresponds to a SFR$_{\rm FIR}$\ssim120\,\sfrU.
These physical properties suggest that the merger system is dusty in nature with active starburst activity.
We also derive a stellar mass of $M^{*}$\ssim3\E{10}\,\Msun by fitting SED models to the
rest-frame UV-to-mm photometry, which have been corrected for their respective magnification factors before performing the fit, in order to
take into account the effect of differential lensing.
% compare intrinsic CO size and stellar
The source-plane distribution of the gas and stellar populations of different ages
indicates that the CO gas is of similar spatial extent as the old and long-lasting stellar populations, whereas
recent \SF regions appear to be embedded within the inner few kpc of the molecular gas reservoir.
The compact \SB in RXJ1131 is likely a consequence of gas accumulation driven by interactions with the companion.
While properties such as CO linewidths, CO size, SFR, and gas mass found in RXJ1131
resemble those of local ULIRGs and high-$z$ starburst galaxies,
its SFE is comparable to those of nearby and high-$z$ disk-like galaxies rather than
\SB systems. %
% This results from the SFR sitting at the low end, and gas mass at the high end among \SB systems
% This is in agreement with its disk-like kinematic signatures.
Recent theoretical simulations have illustrated that the disk component of a gas-rich
...
As such, the extended molecular gas disk of RXJ1131 together with its low SFE may be indications
that the nuclear starburst in RXJ1131 could form a
larger stellar bulge in the merger remnant upon coalescing, and yet, the
inefficient removal of gas angular momentum renders it unlikely
to evolve into an E/S0 galaxy.

Using the CO redshift as the systemic redshift for the host galaxy of RXJ1131, we find
that the optical BLR lines reported in previous studies are blueshifted by $\sim$780\,\kms.
...
Since such an offset is typically $\sim$100\pmm270\,\kms,
we thus raise an alternative explanation of
of a recoiling black hole in RXJ1131, which
would also explain the highly spinning black hole.
...
Adopting the stellar mass as an upper limit to the bulge mass of RXJ1131, we find a
$M_{\rm BH}$$/$$M_{\rm bulge}$$\gtrsim$0.27\%, providing
...
black hole in RXJ1131 grows faster or earlier than its host galaxy than the local counterparts.
This is consistent with existing studies of the evolution in the co-eval growth between SMBHs and their host galaxies.

This work demonstrates that, with the aid of
gravitational lensing and
dynamical lens modeling of resolved spectral line observations,
it is possible to study the gas kinematics and dynamics in high-$z$ quasar host galaxies
% in great details
despite strong quasar emission.
The first use of such technique is presented by \citet{Riechers08a}, who reconstruct a source-plane
velocity gradient and constrain the gas dynamics in a $z$\,$>$\,4 quasar PSS J2322$+$1944,
