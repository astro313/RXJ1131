...
\caption{
An overlay of the velocity-integrated \bco emission on the {\it HST} V$-$band (F555W)
...
$\sim$100,000 samples as the initial ``burn-in'' stage to stabilize the Markov
...
H-Band, V-band, and I-band (C06), where the
...
differential lensing is also non-negligible for CO, where the red wing
of \bco is apparently much brighter than that in the blue wing, consistent
...
Assuming the velocities of the respective lens models
correspond to solely the tangential component (i.e., $\theta$ = 0), we extract
a pseudo PV diagram along the major axis in \Fig{PV} by slicing across their
source plane positions. The best-fit major axis is along PA of 121\degr.
...
of 500 iterations, which we perturb the input parameters according to a random
Gaussian distribution of sigma corresponding to their uncertainties.
...
of the rotation curve, the asymptotic velocity is poorly constrained.
...
Studies of the Tully-Fisher relation typically use V$_{2.2}$ (velocity
at 2.2 disc scale-length or $\sim$1.375 half-light radius,
or $\sim$0.7$R_{\rm opt}$) as the rotation velocity ($V_{\rm max}$ in their
terminology), which corresponds to the radius at which the velocity
...
our cosmology), who use all {\it HST} H--, V--, and I--bands in their
modeling. If only V$-$band is used, the half-light radius is $\sim$4kpc \citep{Brewer08a}.
This discrepancy arises owning to the exclusion of the more diffused emission in the V-band
(\ie V-band is missing the diffused emission).
This indicates that the molecular gas may be ... (see \Sec{size}). DR: ok, let's discuss this.
...
\subsection{Gas Mass}
\subsection{Source Extent} \label{sec:size}
Compared to HST:
