...
The lensing-corrected molecular gas masses are $M_{\rm gas}$\ssim1.4\E{10}\,\Msun
and \ssim2\E{9}\,\Msun for RXS\,J1131$-$1231 and the companion, respectively.
The gas mass fraction of $\sim$18\% inferred for RXS\,J1131$-$1231 is consistent with the trend of decreasing molecular gas fraction since $z$\ssim2 reported in recent studies.
We find lensing-corrected stellar and dust masses, an IR luminosity, and a
star formation rate of
$M_{*}$\ssim3\E{10}\,\Msun, $M_{\rm dust}$\ssim3\E{8}\,\Msun, $L_{\rm IR}$\ssim1.5\E{12}\,\Lsun,  and SFR$_{\rm FIR}$\ssim120\,\sfrU for RXS\,J1131$-$1231.
...
($\nu_{\rm rest}$\,=\,$230.53800$ GHz) at the tuning frequency of 
...
% redshifted to 
at the observe frequency of $\nu_{\rm obs}$\eq209.10443\,GHz    % i.e. where we thought the line was redshifted to 
...
The observed properties of \bco and \cco emission are summarized in \Tab{obsprop}.
\input{table_obsprop}

\subsection{\bco Emission} \label{sec:CO21}
We detect \bco line emission toward the background source in the PdBI data
at $\gtrsim$\,27$\sigma$ significance, enabling us to constrain its redshift at 
$z_{\rm CO}$\eq0.6541\,$\pm$\,0.0002\footnote{This redshift is derived by fitting a double-Gaussian to 
the de-lensed spectrum, which is shown in \Fig{delensed}.
% instead of the observed spectrum (\Fig{CO21spec}) to avoid biases in our 
% redshift determination of the dynamical center due to differential lensing (see \Sec{differential}).
...
to the integrated line image yields 5\farcs1\pmm0\farcs7$\times$3\farcs7\pmm0\farcs7, 
consistent with the results obtained by $uv$-plane fitting within the uncertainties. 
% UV plane: E_GAUSS: 4.5 +/- 0.1 x 3.5 +/- 0.2
To model the complex spatial distribution of the line emission (\Fig{CO21mom}), we 
fit two independent Gaussians and find deconvolved source sizes of 
3\farcs8\pmm0\farcs4$\times$1\farcs9\pmm0\farcs4 and 3\farcs6\pmm0\farcs3$\times$1\farcs5\pmm0\farcs3.
Therefore, the emission is slightly resolved over $\sim$\,2 beams.
The lensed CO emission is unlikely to be distributed as Gaussians 
given the morphology seen in optical and \nir emission that were observed at higher resolution, 
i.e., the lensed emission of CO (without beam convolution) is more likely to have 
an annulus configuration enclosing a partially complete ``Einstein ring'' with 
structures along the arc 
and knots (see \Fig{CO21mom}). Nevertheless, the deconvolved source sizes and the
spatially offset blue wing compared to optical emission suggest that 
the CO-emitting region in RXJ1131 is more extended than its stellar (and of course quasar) emission.
...
35.7\,$\pm$\,6.9\,Jy\,\kms by summing up fluxes over the FWZI
...
$r_{\rm 32}$\,=\,$T_{\small \cco}$$/$$T_{\small \bco}$\,=\,0.78\,$\pm$\,0.62, where this uncertainty includes 
those introduced in absolute flux calibration and is dominated by 
the large phase errors associated with the \cco observations.
...
The velocity scale is with respect to $z$=0.6541, which corresponds to the
line center of \bco line emission. The spectral resolution for \cco and \bco
...
We therefore adopt $S_{\nu}$\eq0.39\pmm0.22 mJy as the 2\,mm continuum flux measurement for 
the background galaxy (RXJ1131). 
The quoted uncertainty is solely based on the uncertainties associated with the flux density of the 
point source model ($\delta S_{\nu}$\eq0.088\,mJy) and 
that of the integrated flux in the original data (0.2\,mJy), and does not account for that in the 
...
Contours correspond to [10, 40, 60, 100]\%$\times$ the peak flux density of $S_{\rm 5GHz}$\eq0.74\pmm0.01 mJy.
% Radio emission towards the foreground radio core is detected at $\gtrsim$57$\sigma$,
% where $\sigma_{\rm 5GHz}$\,=\,13 $\mu$Jy beam\pmOne.
...
We find a spectral index of $\alpha^{\rm 2mm}_{\rm 6cm}$\,=\,$-$0.02
for the foreground galaxy and $\alpha^{\rm 2mm}_{\rm 6cm}$\,=\,$-$0.35
for the background galaxy by fitting
power-laws ($S_\nu \propto \nu^{\alpha}$) to their continuum emission at
5\,GHz and 2\,mm. 
The spectral slope derived for the background source is flatter than the typical slope of pure synchrotron emission ($\alpha$\ssim$-$0.7), suggesting 
that a dominant fraction of the observed 2\,mm emission arises from cold dust and free-free emission. 
% that the 2\,mm emission is dominated by non-synchrotron emission.
This spectral slope would only be shallower 
if a non-negligible fraction from the background source 
contribute to the point source flux at 2\,mm in the deblending process. This would also 
explain the atypical flat spectrum found for the foreground galaxy. 
Assuming a synchrotron slope of $\alpha$\ssim$-$0.7 and that the emission at 5\,GHz is purely synchrotron, 
we find $S_{\rm 2mm}$\eq0.27\pmm0.22\,mJy from cold dust and free-free emission in the background galaxy. 
...
Top: each panel corresponds to a channel map of width 107.5\,\kms around the annotated central velocity where
...
magnification factor varies from 10.9 to 7.8 \citepalias{Claeskens06a}.
Here, we find that the highly asymmetric \bco line profile is predominantly a result of 
differential lensing, where the redshifted emission is apparently much brighter than the
blueshifted component, as indicated by the difference in their magnification factor $(\mu_{\rm L})$, which
varies from 8.7 to 3.1 across the \bco line (\Tab{model}).
A secondary effect contributing to the asymmetry is caused by 
the CO emission in the companion in the redshifted velocity channels.
...
As discussed in \Sec{caveat}, we interpret RXJ1131 as a disk galaxy given 
its reconstructed source-plane locations of various kinematic components displaying
a velocity gradient with a symmetric double-horned line profile (\Fig{delensed}, \Fig{model} and, left panel of \Fig{PV}), 
and the disk-like morphology found for source-plane reconstructed optical-NIR emission
...
by slicing across their source plane positions along this major axis (PA: 121\degr).
...
for the PdBI 2\,mm continuum in our fitting procedure. 
Here, we use a flux density of $S_{\rm 2mm}$\eq0.27 mJy derived in \Sec{deblend} 
...
dust mass of 16\petm{5}{12}\E{8}\,\Msun, all of which are uncorrected for lensing magnification.
...
dust mass $M_{\rm dust}$ of 14\petm{5}{7}\E{8}\,\Msun, all of which are uncorrected for lensing.
...
Offset between BLR and NLR lines have been reported in literature, e.g., 
\citet{Richards02a} find a
median of 97\kms and a dispersion of 269\,\kms in the distribution of
the offset between [Mg\,{\scriptsize II}] and [O\,{\scriptsize III}] lines in an SDSS sample of $>$3800 quasars, 
and \citet{Bonning07a} find a mean offset of 100\,\kms with a dispersion of 212\,\kms %FWHM = 500
between the broad component of
H$\beta$ and \oiii lines in a sample of $\sim$2600 quasars at 0.1$<$$z$$<$0.81,
illustrating that large offsets ($\Delta v$$\gtrsim$\,500\,\kms) are uncommon but have been observed in some cases.
...
Common interpretations used to explain the offset between optical and systemic redshift include
e.g., outflow/inflow of gas in the BLR, 
where the dominant component of the BLR linewidth
is the rotation of the accretion disk, superimposed by an large inflow component, 
or arising from the inclination angle of the (inner) accretion disk, where 
clumps in the disk that is more edge-on block the far side, and thus only see the redshifted (for RXJ1131) 
inflowing component on the BLR gas.
{\bf DR: is it because of the large linewidth (one component of \mgii is $>$20000\kms, and broad comp. of H$\beta$ is $\sim$2000$-$4000\kms) that the referee thinks it's more likely to be inflow and therefore doesn't like the recoil black hole?}
Additionally, microlensing in RXJ1131 \citep[\eg][]{Sluse07a, Sluse12a} may preferentially magnified the (red wing of the compact BLR 
emission), and thus biasing the BLR line centroids.% , i.e., the redshift offset may not be a real one.
% Sluse07: BLR with wide FWHM (> 2000\kms in \mgii and H$\beta$), one component of \mgii > 20000 \kms --> partial attentuation of the blue sides of the line could shift the apparent BLR line centroid to the red side.


%  --------------- note --------------
% recoil...
% Given that there's an accretion disk (Dai+10, Pooley+07).., which could have been reform after the merging (in ~100 year; Bonning+07), then it would mean that the binary merged > blah years (presumably from previous encounter).
% [ignore this] could also be currently a binary orbiting at high speed? (do we see multiple components in the BLR lines? Do we know that there's only a single BH from Xray? probably only 1 AGN, because otherwise we would see more complicated lensed QSO emission)
%
% Sluse07:  "This indicates that the flux in the NEL is partly resolved along the slit direction. On the other hand, we did not find a significant offset (i.e. offset <1 pixel) of the centroids of the NEL images w.r.t. to the centroid of the continuum emission. Using a SIE+? to model the lensing galaxy (Paper I), we calculate that the absence of such an offset implies an upper limit of roughly ?30h?1 pc on the off-centering of the [O III] emission w.r.t. the continuum emission."
% --> so the NLR is only slightly spatially offset from the continuum, and therefore must be inflow and microlensing? 
% --> but couldn't it still be recoil even if it's not spatially offset (perhaps unlikely?) e.g. Allen+15, Section 4.1 "the [O III] peaks do not coincide spatially with the X-ray sources, indicating that they may not be directly due to the two AGN, and suggest a bi-conical outflow as a likely alternative." << sounds like if the NLR is not co-spatial with the AGN, then could be outflow. whereas if referee thinks no spatial offset (Sluse+07) and therefore probably inflow rather than recoil AGN doesn't seem to hold. 
% ----> hmm, but I guess outflow could be spatially offset but not necessary for inflow.
% --------------------------------------

In hierarchical model of galaxy formation, supermassive black holes are formed by
mergers, of which the majority took place at high redshifts. 
% The most commonly used technique to find offset AGNs is to 
% look for shifts between BLRs associated with the QSO and the NLR associated 
% with the starlight from the galaxy. 
We also discuss the possibility of a recoil black hole from previous episode of merger at earlier epoch \citep{Madau04a} leading to the offset... 
Alternatively, the offset between the BLR and the systemic redshift 
may be partially due to a kinematically offset AGN \citep{Hoffman & Loeb (2006)}, where
BLRs are gravitationally bound to
the central AGN (with a BLR dynamical timescale $\sim$10\E{2} years \citep{Bonning07a}, \ie can be quickly regenerated), 
whereas this condition is not necessary for NLRs.
% But Madau04a: a ~1e8\Msun BH in a $\sigma$\eq200\kms galaxy, the decay timescale due to dynamical friction is $<$ 1 Myr $<<$ Salpeter timescale, and so even for a kick velocity of v_CM\eq300\kms, the displacement is quite small $\lesssim$$R_{\rm BH}$\approx$10 pc.
...
A recoiling black hole is expected % once they overcome the ``final pc problem'' by e.g. stellar dynamical hardening/hardening by 
% dynamical friction against background stars, or viscous gas dynamical drag.
...
Depending on their initial conditions (e.g., black hole mass ratio, spin-orbit orientation, spin magnitude), numerical relativity simulations have shown that the recoil velocity can reach up to  $v_{\rm kick}$\ssim4000\,\kms for spinning BHs \citep[\eg][]{Campanelli07a}, 
with a typical offset recoil velocity $v_{\rm kick}$ in the range of 100$-$500\,\kms 
% based on simulations, and simulation constraints from the scatter in the local $M_{\rm BH}$$-$\mbulge relation
\citep[\eg][]{Favata04a,Libeskind06a,Campanelli07a}.
The black hole can therefore either leave the host galaxy (if vkick $>$ $v_{\rm esc}$) or 
oscillate around the center of the galaxy, its orbit decaying because of dynamical friction 
\citep[see \eg \citealt{Volonteri10a} for a discussion]{Bonning07a, Colpi14a}.

% escape velocity
$M_{\rm BH}$/\mstar\ssim1/375, $v_{\rm circ}$\ssim$v_{\rm rot}, then the upper limit of $v_{\rm kick}$\ssim2$\times$$v_{\rm circ}$, 
assuming bulge escape velocity $v_{\rm esc}$\eq2.83$\v_{\rm circ}$ = 858\kms
(in the case of a density profile of the spherical \citet{Hernquist90a} model for the stellar bulge component, 
spherically symmetric isotropic model)%  \citep{BoylanKolchin04a}, 
$v_esc$\eq2sqrt(2) *  $v_{\rm circ}$($r$\eq$a$), where r is the stellar bulge scale length, at most a few kpc, 
therefore finding $v_esc$\eq BLAH\kms.
$t_dyn$ would be 
% Assuming know the rotation curve of RXJ1131 is similar to that MW, then at inner radius, we would expect $v_{\rm rot} (r=a)\gtrsim$ the extended 
% gas disk rotation, given that we don't know precisely the stellar bulge scale length, if we assume 
% $v_{\rm rot} (r=a)$\ssim$v_{\rm rot}$ of the gas disk, then v_esc ~ 860\kms. Therefore BH unlikely to escape anyway.

In the case of gas-rich merger remnants, where large amounts of gas have been
funneled to the central regions find that the MBHs remain within R$<$1\,kpc from the center
of the host even for high recoil velocities ($v_{\rm kick}$\eq1200\,\kms) due to the compactness of the
remnant galaxy?s nuclear disk" \citep{Mayer07a,Guedes08a}.
Several outcomes of the recoiling blackholes include,
a) In spherical bulges the black hole will oscillate right through the center, and return to the center after a few dynamical times, or 
deplete or "heat up" the stars in the nucleus, reducing the efficiency of dynamical and extending the return time.
% in gas-rich mergers, gas drag, duty cycle of the wandering AGN..
% depending on the nuclear disk density profile, the MBH is unlikely to reach large distances
%  the timescale for the recoiling hole to return to the center of its host galaxy depends very sensitively on the magnitude of the kick it receives, ranging from $\sim$10\E{6} yr for v_CM\eq200 \kms to more than 109 yr for v_CM \eq 400 \kms \citep{Madau04a} 
% Loeb07, Madau04a, Hoffman07: At low ejection speeds, the deceleration of the BH by the external gravitational potential of the galaxy and by dy- namical friction on the background stars and gas would limit the displacement of the slow-moving BH and tend to bring it back to the center of its host galaxy within a few dynamical times
b) If the black hole is able to leave the bulge / ejected and enters a triaxial dark matter halo, it may not return at all within a Hubble time \citep{Guedes09a} and become intergalactic/naked black holes (but rare unless in recoil velocity $>>$ few thousands \kms, i.e., sufficient to escape the host galactic potential) \citep[\eg][]{Loeb07a}.
% Volonteri+10: ejected BHs are rare at $z$$<$5, especially in wet-mergers
%c) High resolution simulations of gas mergers (e.g. Mayer et al. 2007) show that up to 60\% of the initial gas is funneled to the center of the remnant. In that particular simulation, the mass of the nuclear disk is Md = 3e9 Msun, and recoliing black holes with velocities of 
%$~$500 km/s are not able to escape the central region (scale length L=75 pc).
...
SDSSJ0927+2943 at $z$\,$\sim$\,0.713, where a set of redshifted narrow lines and a second set of blueshifted broad AGN lines
have been observed \citep{Komossa08a}.
However, this scenario for SDSSJ0927+2943 was recently refuted by \citet{Decarli14a}, who find that the
BLR lines in SDSSJ0927+2943 are indeed consistent with the CO systemic redshift.
This is in contrast with RXJ1131, where our CO observations confirm the redshifted BLR.
Since this model requires a merged BH, it would imply that RXJ1131 might
have been in a previous merger, which might explain the highly spinning BH in RX1131 \citep[$a$\ssim0.9;][]{Reis14a}. 
% will spin up if major merger but would spin down in minor merger \citep{Gammie04a}, in general would spin down after merging.
% Dave Chernoff: can spin up after merging
The available data do not provide direct evidence favoring any of the above scenarios, 
we therefore cannot rule out any of the above plausible scenarios; $\simless$ 1-kpc
resolution observations probing the morphology, structures and kinematics of the (ionized) gas and stellar (e.g., via H$\alpha$ kinematics measurements) would be useful to distinguish between these scenarios.  
% I think sluse+07 constrained the size of BLR based on model and not really resolving down to those spatial resolution? e.g. not really see outflows
