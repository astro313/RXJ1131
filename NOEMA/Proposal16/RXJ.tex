...
\vspace{0.3em}
\noindent {\bf This proposal} \\ % what can we learn
\indent 
Based on scaling relations 
% for the Toomre scale length 
in \citet{Genzel11a}, % , L\_toomre ~ f\_gas *R\_disk, then
...
Studies have shown that resolutions corresponding to $\sim$1.7$-$2\,kpc are sufficient to withstand %tolerate
beam smearing effect % and still resolve disk model 
to extract rotation velocity and 
dispersion 
% for 
% \vs and kinematic analysis 
...
% By comparing 
% this observed clumps with the theoretical Jeans length, we can 
...
Given that structures of the lensed emission is clearly resolved in our VLA radio map at $\sim$0.4'', % Although what I am actually thinking of the ALMA in mind
...
% o reconstruct the source-plane velocity structures, thereby  % reconstructing the source-plane spatial distribution of the gas and its velocity field
...
compare to its population at other redshifts. % and dependence on its gas properties.
...
ensure the diffuse lensed emission on $\gtrsim$0.47'' scale is not resolved out. % just A array
...
{\em Left:} Extended CO emission with structures as shown in
\bco intensity map overlaid on an optical image.
{\em Middle:} 
...
Our dynamical modelling is currently limited by the poorly constrained rotation profile.
