...
\hspace{-1.5em}
\includegraphics[trim= 10 0 0 0, clip, scale=0.32]{Fig/Manipulate_figsCROP}
\vspace{-1em}
}
\end{figure}
...
for theoretical models as it entails the physical processes 
...
locally in \Lp[\dhcn]-\LIR and \Lp[\dhcop]-\LIR \citep[\Fig{SF}; ][]{Zhang14a} at higher redshifts.
%
\begin{figure*}[!tbph]
\centering
\includegraphics[trim=15 0 5 10, clip, width=0.23\textwidth]{Fig/Zhang}
\includegraphics[trim=5 5 5 10, clip, width=0.23\textwidth]{Fig/Zhang2}
\includegraphics[trim=10 0 0 5, clip, width=0.4\textwidth]{Fig/Greve14Fig2}
\caption{ \fontsize{10pt}{12pt}\selectfont {\textbf{Integrated form of the SF law: log \LFIR = $\alpha$ log \Lp[X]+$\beta$.}}
{\em Left \& middle:}
A linear correlation between \LIR-\dhcn and \LIR-\dhcop have been established toward nearby galaxies 
with \LIR$\sim$10$^9$-10$^{12}$\Lsun \citep{Zhang14a}, demonstrating the utility of mid-$J$ lines in tracing 
...
\begin{figure*}[tbph]
\centering
\includegraphics[trim=25 6 5 5, clip, width=0.75\textwidth]{Fig/combine}
\caption{ \fontsize{10pt}{12pt}\selectfont {}
% CO2-1 line profile
{\em (a):} \textbf{PdBI \rot[CO]{2}{1} dynamically resolved line profile}
The \bco spectrum showing a double-horned, highly asymmetric line profile, which arises due to 
differential lensing across the kinematic components of the source (see (f)).
% HST
{\em (b):}  \textbf{Detection of a gas-rich merger with spatial variations traced by \bco}
The (red, green, blue) contours correspond to the velocity-integrated 
line flux of different velocity channels (red wing, line center, blue wing) in our \rot[CO]{2}{1} data. 
{\em (c):} \textbf{RXJ1131-1231 kinematics in \bco}
% 1st order
The observed velocity gradient is suggestive of a kinematically ordered disk at 
the current resolution limit.
{\em (d):} \textbf{RXJ1131-1231 dynamics in \bco}
% 2nd order
...
% RGB spectra
{\em (e): } \textbf{Differential lensing with spatial variations}
Overlay of spectra taken at regions along the strongest velocity gradient, demonstrating 
differential lensing of the different kinematic components of the galaxy. 
% Lens model
{\em (f): } \textbf{Dynamical lens model}
...
Given the presence of a companion galaxy within 2.4 kpc, high-resolution 
...
We here aim to spatially and dynamically resolve the dense, excited gas emission, 
which will allow us to unambiguously examine the dynamic structure of the ISM in 
the distant merger and provide observational constraints on the star-formation and 
