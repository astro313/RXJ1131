...
is steeply rising ($z$$\sim$0.3-1) is critical and we here aim to bridge this gap by testing
...
their extreme star formation rates are a natural consequence of either gas is being converted into stars
more efficiently and/or their increased molecular gas content. Fragmentation
of giant star-forming clumps and turbulent conditions are thus expected 
...
Resolving the gas dynamics on sub-kpc scales is therefore an important first step
to understanding the mechanisms and physical processes taking
place on different scales and how the ISM conditions 
...
proposed as
%  diagnostic tools 
a diagnostic tool to reveal deeply buried AGNs at the cores of ULIRGs \citep{Imanishi14a, GB14a, Viti14a, Izumi16a, Imanishi16a}.
...
seven distinct structures in the source plane (\Fig{HST}). Remarkably, emission from 
a spatially offset region ($\sim$2.4 kpc away from the AGN host galaxy) has been identified and found 
to be $\sim$700 pc in size \citep{Brewer08a}, indicating a companion galaxy. 
...
dynamical lens model of the system, probing at sub-kpc scale resolution % probe down to 0.15" <-> 500 pc
(about the typical size of high-z GMCs), which will enable us 
...
which are indications of the evolutionary stage of this galaxy.
...
SB regions ($>$kpc scales) in the host galaxy, and by measuring spatial variations in 
...
inner disks to initiate SB/AGN activities and how the ISM differs from 
...
\hspace{-0.65em}
\centering
\floatbox[{\capbeside\thisfloatsetup{capbesideposition={right,center},capbesidewidth=0.625\textwidth}}]{figure}[\FBwidth]
...
beam, where an elevated line ratio is seen in AGN and circumnuclear regions relative to starbursts.
...
measurements toward the AGN and the extended SB regions in order to investigate spatial variations 
and assess its utility as an AGN/SB diagnostic at high redshift.
...
% 
% XDR versus PDRs
Since \eco is excited at higher temperature than the HCN and \hcop lines (e.g. in 
...
% physical properties
In addition, we will use spatially resolved line ratios of HCN/CO and \hcop/CO within the AGN host galaxy
as proxies of its very dense (cores; $\sim$10$^5$-10$^7$ cm$^{-3}$) versus the less dense (clumps; $\sim$10$^4$
cm$^{-3}$) gas content and their spatial distributions as a function of distance from the AGN. This will enable
us to gain insight into the SF mode on small scales across different galaxy populations, 
as traced by the HCN/CO-\LIR relation, and investigate
the interplay between AGN, SB and the dense gas content in the ISM.
% \Fig{SF}).
% CO SLED
We will also combine \eco with our existing \bco and CO($J$=3\rarr2) data to constrain the gas density ($n_{\rm H_{2}}$) 
...
This will enable us to provide crucial constraints on the conditions for star-formation models at high redshift.

\medskip
\noindent In summary, our proposed observations therefore provide an exceptional opportunity to investigate
the physical properties and dynamical structures of different gas phases in the ISM of a distant merger
...
\includegraphics[trim=15 0 5 10, clip, width=0.25\textwidth]{Fig/SFRSD}} 
{
\hspace{-0.54em}
\caption{ \fontsize{10pt}{12pt}\selectfont{
...
\floatbox[{\capbeside\thisfloatsetup{capbesideposition={right,center},capbesidewidth=0.445\textwidth}}]{figure}[\FBwidth]
{
\includegraphics[trim=37 35 247 5, clip, width=0.575\textwidth]{Fig/combine.pdf}}
{
\hspace{-1.0em}
